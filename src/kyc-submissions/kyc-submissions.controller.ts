import { Controller, Get, Patch, Query, Param, ParseIntPipe, Body } from '@nestjs/common';
...
export class KycSubmissionsController {
  ...
  @ApiQuery({ name: 'limit', required: false, description: 'Number of submissions per page (default: 10, max: 100)', example: 10 })
  @ApiQuery({ name: 'search', required: false, description: 'Search term for user details, ID number, or nationality', example: 'john' })
  @ApiQuery({ name: 'status', required: false, description: 'Filter by KYC status: pending, processing, approved, rejected', example: 'pending' })
  @ApiQuery({ name: 'level', required: false, description: 'Filter by KYC level: 1, 2, 3', example: '1' })
  @ApiResponse({
    status: 200,
  ...
  async getKycSubmissions(
    ...
    return this.kycSubmissionsService.getKycSubmissions(validPage, validLimit, search, status, level);
  }

  @Get(':id')
  @ApiOperation({ summary: 'Get KYC submission by ID' })
  @ApiParam({ name: 'id', description: 'KYC submission ID', example: 'clx123kyc456789' })
  @ApiResponse({
    status: 200,
  ...
  @ApiBody({
    schema: {
      type: 'object',
      properties: {
        status: { type: 'string', enum: ['PENDING', 'PROCESSING', 'APPROVED', 'REJECTED'], example: 'APPROVED' },
        reviewedBy: { type: 'string', example: 'admin_user' },
        rejectionReason: { type: 'string', example: 'Document quality is too poor' }
      },
      required: ['status']
    }
  })
  @ApiResponse({
    ...
    schema: {
      ...
      properties: {
        ...
        message: { type: 'string', example: 'KYC submission for john_doe approved successfully' }
      }
    }
  })
  @ApiResponse({
  ...
  async updateKycSubmissionStatus(
    @Param('id') id: string,
    @Body() body: { status: string; reviewedBy?: string; rejectionReason?: string }
  ) {
